...
uint256 hashGenesisBlock("0xa28dcb138cef4fa6ea6e1a49fafb352c72c4fffa6674f2bb7e506817e27bdfcc");
...
static const int64 nTargetTimespan = 5 * 60 * 2 * 30; // Globalcoin: Every 600 Blocks.
static const int64 nTargetSpacing = 1 * 40; // Globalcoin: 30 minute blocks
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    int64 nDiffSwitchBlock = 179860;
    int nHeight = pindexLast->nHeight + 1;
    bool fNewDifficultyProtocol = (nHeight >= nDiffSwitchBlock || fTestNet);
    
    //Starting at block 179860 retarget every block with exponential moving toward target spacing
...
unsigned int static KimotoGravityWell(const CBlockIndex* pindexLast, const CBlockHeader *pblock, uint64 TargetBlocksSpacingSeconds, uint64 PastBlocksMin, uint64 PastBlocksMax) {
    ...
    if (BlockLastSolved == NULL || BlockLastSolved->nHeight == 0 || (uint64)BlockLastSolved->nHeight < PastBlocksMin) { return bnProofOfWorkLimit.GetCompact(); }
        ...
        for (unsigned int i = 1; BlockReading && BlockReading->nHeight > 0; i++) {
                ...
                if (BlockReading->nHeight > 2001000 && LatestBlockTime < BlockReading->GetBlockTime()) {
                   //eliminates the ability to go back in time, we need to change this block height to past the kgw
                   LatestBlockTime = BlockReading->GetBlockTime();
               }
                PastRateActualSeconds                        = BlockLastSolved->GetBlockTime() - BlockReading->GetBlockTime();
                PastRateTargetSeconds                        = TargetBlocksSpacingSeconds * PastBlocksMass;
                PastRateAdjustmentRatio                        = double(1);
                if (BlockReading->nHeight > 2001000){
                    //this should slow down the upward difficulty change, we need to change the block height after kgw
...
{
        static const int64        BlocksTargetSpacing                        = 1 * 40; // 40 Second Blocks 
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
        int DiffMode = 1;
        if (fTestNet) {
                if (pindexLast->nHeight+1 >= 1000) { DiffMode = 2; } // We need to set up Testnet
        }
        else {
                if (pindexLast->nHeight+1 >= 2000000) { DiffMode = 2; } // We need to add the block height to fork it from.
        }
...
bool LoadBlockIndex(bool fAllowNew)
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xa28dcb138cef4fa6ea6e1a49fafb352c72c4fffa6674f2bb7e506817e27bdfcc");
    }
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Julia Gillard Rolled as PM";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 13722631470; //epochtime
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 357826;

        if (fTestNet)
        {
            block.nTime    = 13722631470;
            block.nNonce   = 357826;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xd7dff5b5b016b510ee107fe83b0a62abd93ff24c9525bb57caaca86a3cae5877"));
